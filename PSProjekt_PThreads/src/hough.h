...
void blur(FuncArgs *args);
void keepMaximas(FuncArgs * args);
double** getMaximas(FuncArgs * args);
...
double** removeOutOfBorderIntersections(FuncArgs*args, double** isects){
	...
	double** visibleISects=(double**)malloc((allISects+1)*sizeof(double*));
	visibleISects[0]=(double*)malloc(sizeof(double));
...
double** getLines(FuncArgs * args) {
	...
	double** maximas= getMaximas(args);
	int numMaximas=args->numLines;
	args->lines=maximas;
	keepMaximas(args);
	maximas=getMaximas(args);
	...
	for (; i < end; i++) {
		...
		for (; j < cols; j++) {
			...
			if (htr[i][j] > htr[i - 1][j - 1] && htr[i][j] > htr[i - 1][j]
				...
				lines = (double**) realloc(lines,((int)lineCount + 1) * sizeof(double*));
				lines[lineCount] = (double*) malloc(sizeof(double) * 3);
				lines[lineCount][0] = j;
				lines[lineCount][1] = i;
...
double** makeDoublearray(int x, int y) {
	double **ret = (double**) malloc(x * sizeof(double));
	int i = 0;
	for (; i < x; i++) {
		ret[i] = (double*) malloc(y * sizeof(double));
		int j = 0;
		for (; j < y; j++) {
			ret[i][j] = 0;
		}
